 (function () {
      ...
      function hashCode(s) {
        if (!s.length) return 0; let hash = 0; for (let i = 0; i < s.length; i++) { hash = (hash << 5) - hash + s.charCodeAt(i); hash |= 0; } return hash;
      }
      function addKeywords(source, keywords) {
        if (!keywords) return source; let head = ''; for (const kw of keywords) head += `#define ${kw}\n`; return head + source;
      }
      ...
      const displayShaderSource = `
        ...
        uniform sampler2D uTexture;
        uniform sampler2D uDithering; // not used but kept for parity
        uniform vec2 ditherScale;      // not used
        uniform vec2 texelSize;
        void main () {
      ...
      const vorticityShader = compileShader(gl.FRAGMENT_SHADER, `
        ...
        void main () {
          ...
          vec2 velocity = texture2D(uVelocity, vUv).xy; velocity += force * dt; velocity = min(max(velocity, -1000.0), 1000.0);
      ...
      const pressureShader = compileShader(gl.FRAGMENT_SHADER, `
        ...
        void main () {
          ...
          float divergence = texture2D(uDivergence, vUv).x; float pressure = (L + R + B + T - divergence) * 0.25;
      ...
      function getResolution(resolution) {
        ...
        const aspectRatio = w / h; let aspect = aspectRatio < 1 ? 1 / aspectRatio : aspectRatio;
      ...
      function render(target) {
        gl.blendFunc(gl.ONE, gl.ONE_MINUS_SRC_ALPHA); gl.enable(gl.BLEND); drawDisplay(target);
      }
      ...
      function splatPointer(pointer) {
        const dx = pointer.deltaX * config.SPLAT_FORCE; const dy = pointer.deltaY * config.SPLAT_FORCE;
   ...
   const fluidSection = document.querySelector('.fluid-section');

function getRelativePosition(e) {
  const rect = fluidSection.getBoundingClientRect();
  let clientX, clientY;
...
const pointer = pointers[0]; // use the first pointer
...
fluidSection.addEventListener('mouseup', () => {
...
fluidSection.addEventListener('mousemove', (e) => {
  ...
  splatPointer(pointer); // splat even on hover
});

// --- Touch ---
fluidSection.addEventListener('touchstart', (e) => {
  const { x, y } = getRelativePosition(e);
  updatePointerDownData(pointer, e.touches[0].identifier, x, y);
  clickSplat(pointer);
  updateFrame();
}, { passive: false });

fluidSection.addEventListener('touchmove', (e) => {
  const { x, y } = getRelativePosition(e);
  updatePointerMoveData(pointer, x, y, pointer.color);
  e.preventDefault(); // prevent scrolling
}, { passive: false });

fluidSection.addEventListener('touchend', () => {
  updatePointerUpData(pointer);
});
      ...
      // Note: we start the loop on first mousemove/touch to save cycles. You can uncomment to auto-start:
      updateFrame();
    })();
